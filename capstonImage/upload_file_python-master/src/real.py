...
#     return render_template("complete.html", image_name=filename, destination=destination)
#
# #
# # @app.route('/blurring')
# # def blurring():
...
def upload():
    ...
    # return send_from_directory("images", filename, as_attachment=True)
    return redirect(url_for('send_image', image_name=filename, destination=destination,filename = filename, destination2=destination2) )
...
def send_image(filename):
    ...
    model.load('pconv_imagenet.h5', train_bn=False)
    # model.summary()
    print("send_image_destination", {destination} )
    # img = cv2.imread(sys.argv[1], cv2.IMREAD_COLOR)
